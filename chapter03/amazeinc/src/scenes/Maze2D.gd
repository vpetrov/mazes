...
func _input(event) -> void:
    if event is InputEventMouseButton and event.is_pressed():
        var gridCoordinates = mousePositionToCell(event.position)
        var col = gridCoordinates.x
        var row = gridCoordinates.y
        if col >= 0 && row >= 0:
            onGridClick(grid, row, col)

# returns a pair of (col, row) map grid coordinates
func mousePositionToCell(mousePosition:Vector2) -> Vector2:
    var localMousePosition:Vector2 = $TileMap.to_local(mousePosition)
    return $TileMap.world_to_map(localMousePosition)
