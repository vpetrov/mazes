...
onready var polarMaze = preload("res://src/scenes/PolarMaze.tscn")
onready var hexMaze = preload("res://src/scenes/HexMaze.tscn")
onready var triangleMaze = preload("res://src/scenes/TriangleMaze.tscn")
...
onready var mazeTypeSelect = $PanelContainer/MarginContainer/HBoxContainer/MazeTypeSelect
onready var mazeSelect = $PanelContainer/MarginContainer/HBoxContainer/GridMazeGroup/ViewSelect
onready var algorithmSelect = $PanelContainer/MarginContainer/HBoxContainer/GridMazeGroup/AlgorithmSelect
onready var rowsTextEdit = $PanelContainer/MarginContainer/HBoxContainer/GridMazeGroup/RowsTextEdit
onready var columnsTextEdit = $PanelContainer/MarginContainer/HBoxContainer/GridMazeGroup/ColumnsTextEdit
onready var deadEndsLabel = $PanelContainer/MarginContainer/HBoxContainer/GridMazeGroup/DeadEndsLabel
onready var maskedMazeButton = $PanelContainer/MarginContainer/HBoxContainer/GridMazeGroup/NewMaskedMazeButton
onready var maskPanel = $PanelContainer/MarginContainer/HBoxContainer/GridMazeGroup/MaskPopupPanel
onready var maskGrid = $PanelContainer/MarginContainer/HBoxContainer/GridMazeGroup/MaskPopupPanel/MaskGrid
onready var gridMazeGroup = $PanelContainer/MarginContainer/HBoxContainer/GridMazeGroup
onready var polarMazeGroup = $PanelContainer/MarginContainer/HBoxContainer/PolarMazeGroup
onready var polarRowsTextEdit = $PanelContainer/MarginContainer/HBoxContainer/PolarMazeGroup/PolarRowsTextEdit
...
export var polar_rows:int = 20

var grid = null
var currentMazeType = MazeType.GRID
...
const mazeType = [
    { "text": "Grid", "type": MazeType.GRID }, 
    { "text": "Polar", "type": MazeType.POLAR },
    { "text": "Hex", "type": MazeType.HEX },
    { "text": "Triangle", "type": MazeType.TRIANGLE}
]
...
func _ready() -> void:
    ...
    initPolarSize(polar_rows)
    _on_MazeSelect_item_selected(0)
    algorithmSelect.select(5)

func initDisplayOptions() -> void:
    for i in range(mazeType.size()):
        mazeTypeSelect.add_item(mazeType[i].text, i)
...
func initPolarSize(rows:int) -> void:
    polarRowsTextEdit.text = str(polar_rows)
...
func switchViewportScene(scene:PackedScene) -> void:
    disconnect("change_grid", currentScene, "onGridChanged")
    disconnect("show_distances", currentScene, "onShowDistancesChanged")
    disconnect("show_dead_ends", currentScene, "onShowDeadEnds")
...
func _on_NewMazeButton_pressed() -> void:
    match currentMazeType:
        MazeType.GRID: createGridMaze()
        MazeType.POLAR: createPolarMaze()
        MazeType.HEX: createHexMaze()
        MazeType.TRIANGLE: createTriangleMaze()
    
func updateMazeSize() -> void:
    self.rows = rowsTextEdit.text.to_int()
    self.columns = columnsTextEdit.text.to_int()
    self.polar_rows = polarRowsTextEdit.text.to_int()
...
func _on_MazeTypeSelect_item_selected(index: int) -> void:
    changeMazeType(mazeType[index].type)
    
func changeMazeType(type) -> void:
    match (type):
        MazeType.GRID: switchToGridMaze()
        MazeType.POLAR: switchToPolarMaze()
        MazeType.HEX: switchToHexMaze()
        MazeType.TRIANGLE: switchToTriangleMaze()
    pass
    
func switchToGridMaze() -> void:
    currentMazeType = MazeType.GRID
    gridMazeGroup.show()
    polarMazeGroup.hide()
    switchViewportScene(colorMaze)
    _on_NewMazeButton_pressed()
    
func switchToPolarMaze() -> void:
    currentMazeType = MazeType.POLAR
    gridMazeGroup.hide()
    polarMazeGroup.show()
    switchViewportScene(polarMaze)
    createPolarMaze()
    
func switchToHexMaze() -> void:
    currentMazeType = MazeType.HEX
    gridMazeGroup.show()
    polarMazeGroup.hide()
    switchViewportScene(hexMaze)
    createHexMaze() 
    
func switchToTriangleMaze() -> void:
    currentMazeType = MazeType.TRIANGLE
    gridMazeGroup.show()
    polarMazeGroup.hide()
    switchViewportScene(triangleMaze)
    createTriangleMaze()
    

func createGridMaze() -> void:
    updateMazeSize()
    switchAlgorithm(currentAlgorithm)
    updateDeadEnds()

func createPolarMaze() -> void:
    updateMazeSize()
    grid = PolarGrid.new(polar_rows)
    grid.init()
    Maze.recursiveBacktracker(grid, grid.cell(0,0))
    grid.setStartCell(grid.cell(0,0))
    emit_signal("change_grid", grid)
    
func createHexMaze() -> void:
    updateMazeSize()
    grid = HexGrid.new(rows, columns)
    grid.init()
    Maze.recursiveBacktracker(grid, grid.cell(0,0))
    grid.setStartCell(grid.cell(0,0))
    emit_signal("change_grid", grid)

func createTriangleMaze() -> void:
    updateMazeSize()
    grid = TriangleGrid.new(rows, columns)
    grid.init()
    Maze.recursiveBacktracker(grid, grid.cell(0,0))
    grid.setStartCell(grid.cell(0,0))
    emit_signal("change_grid", grid)
