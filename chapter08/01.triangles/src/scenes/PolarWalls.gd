...
func setGrid(grid:PolarGrid) -> void:
    self.grid = grid
    var farthestCell = grid.distances.farthest()
    maxDistance = grid.distances.cells[farthestCell]
    cell_size = getCellSize()
...
func getCellSize() -> float:
    ...
    return visibleArea.x / (grid.nrows * 2.0)
...
func _draw() -> void:
    ...
    for row in range(grid.nrows):
        for col in range(grid.row(row).size()):
...
func draw_cell(cell:Cell, center:int) -> void:
    ...
    if !cell.isLinkedTo(cell.cw):
        var ww = width * 2
        if cell.col == grid.cells[cell.row].size() - 1:
            ww = width
        draw_line(Vector2(cx, cy), Vector2(dx, dy), fg_color, ww, true)
